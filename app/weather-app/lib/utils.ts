...
export const getCurrentWeather = async (searchString : string, units : boolean ) => {
  let resCurrent
  let resForecast
  if (!searchString) {
  ...
  if (Number(searchString)) {
    resCurrent = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?zip=${searchString}&appid=${process.env.NEXT_PUBLIC_WEATHER_API_KEY}&units=${forUnitValue(units)}`
    )
    resForecast = await fetch(
      `https://api.openweathermap.org/data/2.5/forecast?zip=${searchString}&appid=${process.env.NEXT_PUBLIC_WEATHER_API_KEY}&units=${forUnitValue(units)}`
    )
  } else {
    resCurrent = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${searchString}&appid=${process.env.NEXT_PUBLIC_WEATHER_API_KEY}&units=${forUnitValue(units)}`
    )
    resForecast = await fetch(
      `https://api.openweathermap.org/data/2.5/forecast?q=${searchString}&appid=${process.env.NEXT_PUBLIC_WEATHER_API_KEY}&units=${forUnitValue(units)}`
    )
  }
...
export const formatData = ({dataCurrent, dataForecast} : DataProps, units : boolean) => {
  const d = new Date();
  ...
  for (let i = (7 - Math.floor(hours/3)); i < 32 +(7 - Math.floor(hours/3)); i += 8) {
    ...
    for (let a = i; a < i+8 && a < 32 +(7 - Math.floor(hours/3)); a++) {
      ...
      L = dataForecast.list[a].main.temp_min < L ? dataForecast.list[a].main.temp_min : L;
    }
    const dailyTempMax = Math.round(H);
    const dailyTempMin = Math.round(L);
    dailyWeatherData.push({ icon: dailyIcon, day: dailyDay, weather: dailyWeather, highTemp: `${dailyTempMax}${forUnitSymbol(units)}`, lowTemp: `${dailyTempMin}${forUnitSymbol(units)}`});
  }
  ...
  for (let i = 0; i < 9; i++) {
    ...
    hourlyWeatherData.push({icon: hourlyIcon, time: hourlyTime, temp: `${hourlyTemp}${forUnitSymbol(units)}`})
  }
    //data we need to return
  const weatherData : WeatherData = {
    dateHeader : `${wd[dayWeek]}, ${months[month]} ${day} ${year} ${formatTime(hours,minutes)}`,
    nameCity : `${cityName}`,
